...
function displayFood(number) {
  if (number < 0) {
    ...
    console.log(
      ` The ${food[number].name} is a ${food[number].type} and its color is ${food[number].color}`
    );
  } else {
    console.log(
      ` THe ${food[number].name} is a ${food[number].type} and it has colors ${food[number].color[0]} and ${food[number].color[1]}`
    );
  }
}
